...
module.exports = function (connect) {
    ...
    */
    ...
    function SQLiteStore(options) {
        ...
        this.db.exec('CREATE TABLE IF NOT EXISTS ' + this.table + ' (sid PRIMARY KEY,expired, sess); CREATE INDEX IF NOT EXISTS idx_session_expired ON '+ this.table + '(expired)',
            function (err) {
                if (err) throw err;
                self.client.emit('connect');

                dbCleanup(self);
                setInterval(dbCleanup, options.cleanInterval, self).unref();
            }
        );
    }
    ...
    */
    ...
    SQLiteStore.prototype.set = function (sid, sess, fn) {
        try {
            ...
            var now = new Date().getTime();
            var expired = maxAge ? now + maxAge : now + this.options.maxAge;
